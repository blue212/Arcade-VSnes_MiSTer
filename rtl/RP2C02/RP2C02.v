...
always @(posedge Clk) begin
	     ...
	     W0   <= ~ADR[2] & ~ADR[1] & (swap2c05 ? ADR[0] : ~ADR[0]) & ~RnWR & ~nDBER; //  replace W1 and W0 with places for replacing PPUMASK and PPUCTRL RGB PPU RC2C05-01-04
	     W1   <= ~ADR[2] & ~ADR[1] & (swap2c05 ? ~ADR[0] : ADR[0]) & ~RnWR & ~nDBER; //  replace W1 and W0 with places for replacing PPUMASK and PPUCTRL RGB PPU RC2C05-01-04 
...
wire [4:0]ID_DBIN = (PPUtype == 4'h7) ? 5'h1B : //2c05-01 IDs
                    (PPUtype == 4'h8) ? 5'h1D : //2c05-02 3D?
                    (PPUtype == 4'h9) ? 5'h1C : //2c05-03
                    (PPUtype == 4'hA) ? 5'h1B : //2c05-04
                    DBIN[4:0];
// Logics
always @(posedge Clk) begin
...
//	| ({8{R2}} & {R2DB[2:0],DBIN[4:0]}) | ({8{XRB}} & PD_R[7:0]);  // instead of DBIN[4:0] you can add identifiers for 2c05 -02 (03,04)
	| ({8{R2}} & {R2DB[2:0],ID_DBIN[4:0]}) | ({8{XRB}} & PD_R[7:0]); // instead of DBIN[4:0] you can add identifiers for 2c05 -02 (03,04)
...
always @(posedge Clk) begin
	...
	if ( ~R2 ) R2DB7 <= INT_FF;				 
	 ...
	 NFO2      <= ~( BLNK |  H[8] );
	 FAT_IN    <= ~(  H[2]| ~H[1] );	 
	 HBtemp <= (H < 9'd4 || H >= 9'd260); //shifted to get rid of junk on the left 
	 ...
	 if (H_LINE0)  FPORCH_FF <= 1'b1;
         ...
         if (H == 9'd259) BPORCH_FF <= 1'b1;//tweaked because the last pixel was cut off			
...
//PALETTE selector
wire [23:0]RGB_IN_2C02;
wire [23:0]RGB_IN_2C03;
wire [23:0]RGB_IN_2C04_0000;
wire [23:0]RGB_IN_2C04_0001;
wire [23:0]RGB_IN_2C04_0002;
wire [23:0]RGB_IN_2C04_0003;
wire [23:0]RGB_IN_2C04_0004;
wire [23:0]RGB_IN_2C05_99;

PALETTE_2C02 MOD_2C02 ( PIX[5:0], Clk, RGB_IN_2C02[23:0] );
PALETTE_2C03 MOD_2C03 ( PIX[5:0], Clk, RGB_IN_2C03[23:0] );
PALETTE_2C04_0000 MOD_2C04_0000 ( PIX[5:0], Clk, RGB_IN_2C04_0000[23:0] );
PALETTE_2C04_0001 MOD_2C04_0001 ( PIX[5:0], Clk, RGB_IN_2C04_0001[23:0] );
PALETTE_2C04_0002 MOD_2C04_0002 ( PIX[5:0], Clk, RGB_IN_2C04_0002[23:0] );
PALETTE_2C04_0003 MOD_2C04_0003 ( PIX[5:0], Clk, RGB_IN_2C04_0003[23:0] );
PALETTE_2C04_0004 MOD_2C04_0004 ( PIX[5:0], Clk, RGB_IN_2C04_0004[23:0] );
PALETTE_2C05_99 MOD_2C05_99 ( PIX[5:0], Clk, RGB_IN_2C05_99[23:0] );

always @(*) begin
    case (PALETTE)
        4'd0: RGB_IN[23:0] = RGB_IN_2C02[23:0];
        4'd1: RGB_IN[23:0] = RGB_IN_2C03[23:0];
        4'd2: RGB_IN[23:0] = RGB_IN_2C04_0000[23:0];
        4'd3: RGB_IN[23:0] = RGB_IN_2C04_0001[23:0];
